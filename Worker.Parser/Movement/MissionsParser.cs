...
namespace Worker.Parser.Movement
{
    ...
    {
        private MissionType GetMissionType(HtmlNode missionNode) => (MissionType) Convert.ToInt32(missionNode.Attributes.First(a => a.OriginalName == "data-mission-type").Value);
        private DateTime GetArrivalTime(HtmlNode missionNode) => new DateTime(1970,1,1,0,0,0,0).AddHours(2).AddSeconds(Convert.ToInt32(missionNode.Attributes.First(a => a.OriginalName == "data-arrival-time").Value));
        private string GetSourcePlanetName(HtmlNode missionNode) => missionNode.Descendants("td").First(n => n.Attributes.Any(a => a.Value.Contains("originFleet"))).InnerText.Trim();
        private string GetDestinationPlanetName(HtmlNode missionNode) => missionNode.Descendants("td").First(n => n.Attributes.Any(a => a.Value.Contains("destFleet"))).InnerText.Trim();

        private Planet.PlanetPosition GetSourcePlanetPosition(HtmlNode missionNode)
        {
            var positionParts = missionNode.Descendants("td").First(n => n.Attributes.Any(a => a.Value.Contains("coordsOrigin"))).InnerText.Trim().Replace("[", string.Empty).Replace("]", string.Empty).Split(':');
        ...
        {
            var positionParts = missionNode.Descendants("td").First(n => n.Attributes.Any(a => a.Value.Contains("destCoords"))).InnerText.Trim().Replace("[", string.Empty).Replace("]", string.Empty).Split(':');
            return new Planet.PlanetPosition
            {
                Galaxy = Convert.ToInt32(positionParts[0]),
                System = Convert.ToInt32(positionParts[1]),
        ...
        private bool GetIsReturning(HtmlNode missionNode) => Convert.ToBoolean(missionNode.Attributes.First(a => a.OriginalName == "data-return-flight").Value);
        ...
        public async Task<List<MissionBase>> GetFriendlyMissions(HtmlDocument document)
        {
            return await Task.Run(() =>
            {
                ...
                var friendlyNodes = document.DocumentNode.Descendants("tr").Where(n => n.Id.Contains("eventRow") && n.Descendants("td").First().Attributes.Any(a => a.Value.Contains("friendly")));
                    ...
                    if (GetIsReturning(friendlyNode))
                    {
                        friendlyMissions.Add(
                            new FriendlyMission(GetMissionType(friendlyNode),
                                ...
                                new Planet(GetDestinationPlanetName(friendlyNode), GetDestinationPosition(friendlyNode)),
        ...
        public async Task<List<MissionBase>> GetNeutralMissions(HtmlDocument document)
        {
            return await Task.Run(() =>
            {
                ...
                var neutralNodes = document.DocumentNode.Descendants("tr").Where(n => n.Id.Contains("eventRow") && n.Descendants("td").First().Attributes.Any(a => a.Value.Contains("neutral")));
        ...
        public async Task<List<MissionBase>> GetHostileMissions(HtmlDocument document)
        {
            return await Task.Run(() =>
            {
                ...
                var hostileNodes = document.DocumentNode.Descendants("tr").Where(n => n.Id.Contains("eventRow") && n.Descendants("td").First().Attributes.Any(a => a.Value.Contains("hostile")));
